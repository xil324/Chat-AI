  ...
  return (
    <Box sx={{ minHeight: '100vh', background: '#000', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: 6, py: 3, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1.1rem', letterSpacing: '-0.5px' }}>{t('brand')}</Typography>
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', px: 4, textAlign: 'center' }}>
        ...
        <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: { xs: '2.5rem', md: '4rem' }, lineHeight: 1.1, letterSpacing: '-2px', mb: 3, maxWidth: 700, whiteSpace: 'pre-line' }}>
          {t('menu.hero')}
        </Typography>
        ...
        <Box onClick={() => navigate('/ai-chat')}
          ...
          <Box sx={{ flex: 1 }}>
            <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: '1rem', mb: 0.5 }}>{t('menu.aiChat')}</Typography>
            <Typography sx={{ color: '#777', fontSize: '0.875rem', lineHeight: 1.4 }}>{t('menu.aiChatDesc')}</Typography>
          </Box>
      ...
      <Dialog open={open} onClose={() => setOpen(false)}
        ...
        <DialogTitle sx={{ color: '#fff', fontWeight: 700 }}>{t('menu.signOutTitle')}</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ color: '#888' }}>{t('menu.signOutBody')}</DialogContentText>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          <Button onClick={() => setOpen(false)} sx={{ color: '#777', textTransform: 'none', '&:hover': { color: '#fff', background: 'transparent' } }}>{t('menu.cancel')}</Button>
