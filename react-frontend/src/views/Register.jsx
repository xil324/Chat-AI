...
function Register() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!formData.email || !formData.captcha || !formData.password) { setError('Please fill in all fields'); return }
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', background: '#000' }}>
      ...
      <Box sx={{ width: { xs: '100%', md: 480 }, display: 'flex', flexDirection: 'column', justifyContent: 'center', px: { xs: 4, md: 8 }, py: 6 }}>
        <Typography variant="h4" sx={{ color: '#fff', fontWeight: 800, letterSpacing: '-1px', mb: 1 }}>Create account</Typography>
        <Typography sx={{ color: '#777', mb: 4, fontSize: '0.95rem' }}>Fill in your details to get started</Typography>
        ...
        <form onSubmit={handleSubmit}>
          <TextField fullWidth label="Email" name="email" type="email" value={formData.email} onChange={handleChange} required sx={{ mb: 2, ...fieldSx }} />
          <Grid container spacing={1.5} sx={{ mb: 2 }}>
            <Grid item xs={8}>
              <TextField fullWidth label="Verification Code" name="captcha" value={formData.captcha} onChange={handleChange} required sx={fieldSx} />
            </Grid>
            <Grid item xs={4}>
          ...
          <TextField fullWidth label="Password" name="password" type="password" value={formData.password} onChange={handleChange} required sx={{ mb: 2, ...fieldSx }} />
          <TextField fullWidth label="Confirm Password" name="confirmPassword" type="password" value={formData.confirmPassword} onChange={handleChange} required sx={{ mb: 3, ...fieldSx }} />
          ...
          <Typography sx={{ textAlign: 'center', color: '#777', fontSize: '0.875rem' }}>
            ...
            <Link to="/login" style={{ color: '#fff', textDecoration: 'none', fontWeight: 600 }}>Sign in</Link>
          </Typography>
        </form>
      </Box>
    </Box>
