...
function AIChat() {
  ...
  const playTTS = async (text) => {
    try {
      ...
      if (response.data?.status_code === 1000 && response.data.url) {
        ...
        setError(t('chat.error.tts'))
      }
    } catch {
      setError(t('chat.error.ttsApi'))
    }
  }
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data.sessions)) {
        ...
        response.data.sessions.forEach(s => {
          ...
          sessionMap[sid] = { id: sid, name: s.name || `Session ${sid}`, attachedDocumentId: s.attachedDocumentId || null, attachedDocumentName: s.attachedDocumentName || '', messages: [] }
        })
  ...
  const sendMessage = async () => {
    if (!inputMessage.trim()) { setError(t('chat.error.empty')); return }
    const currentInput = inputMessage
    setInputMessage('')
    setCurrentMessages(prev => [...prev, { role: 'user', content: currentInput }])
    setTimeout(scrollToBottom, 100)
    try {
      setLoading(true)
    ...
    <Box sx={{ height: '100vh', display: 'flex', background: '#000' }}>
      ...
      <Box sx={{ width: 240, height: '100vh', borderRight: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ px: 3, py: 2.5, borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1rem', letterSpacing: '-0.5px' }}>{t('brand')}</Typography>
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', px: 1.5 }}>
          <Typography sx={{ color: '#777', fontSize: '0.7rem', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', px: 1.5, mb: 1 }}>{t('chat.recent')}</Typography>
        ...
        <Box ref={messagesRef} sx={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 0 }}>
          {currentMessages.length === 0 && !loading && (
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', minHeight: 400, px: 4, textAlign: 'center' }}>
              <Box sx={{ width: 56, height: 56, borderRadius: '16px', background: 'rgba(99,179,140,0.12)', border: '1px solid rgba(99,179,140,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 3, fontSize: '1.5rem', color: '#63b38c', fontWeight: 700 }}>S</Box>
              <Typography sx={{ color: '#fff', fontWeight: 700, fontSize: '1.4rem', letterSpacing: '-0.5px', mb: 1 }}>{t('chat.welcome')}</Typography>
              <Typography sx={{ color: '#777', fontSize: '0.9rem', maxWidth: 360 }}>
                {attachedDoc ? t('chat.withDoc', { filename: attachedDoc.filename }) : t('chat.welcomeSub') + ' ' + t('chat.attachPdf')}
              </Typography>
            </Box>
          )}
          <Box sx={{ flex: 1, maxWidth: 760, width: '100%', mx: 'auto', px: 4, py: 4, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {currentMessages.map((message, index) => (
              <Box key={index} sx={{ display: 'flex', gap: 2.5, alignItems: 'flex-start', flexDirection: message.role === 'user' ? 'row-reverse' : 'row' }}>
                <Box sx={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 700,
                  ...
                  {message.role === 'user' ? t('chat.youLabel')[0] : 'S'}
                </Box>
                <Box sx={{ flex: 1, maxWidth: message.role === 'user' ? '75%' : '100%' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.75, justifyContent: message.role === 'user' ? 'flex-end' : 'flex-start' }}>
            ...
            {loading && (
              <Box sx={{ display: 'flex', gap: 3, alignItems: 'flex-start' }}>
                <Box sx={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 700, background: 'rgba(99,179,140,0.18)', color: '#63b38c', border: '1px solid rgba(99,179,140,0.3)' }}>S</Box>
                <Box sx={{ pt: 0.5 }}>
                  <Typography sx={{ color: '#63b38c', fontSize: '0.78rem', fontWeight: 700, mb: 1.5 }}>{t('chat.assistantLabel')}</Typography>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
              <FormControl size="small" sx={{ minWidth: 130,
                '& .MuiOutlinedInput-root': { color: '#aaa', borderRadius: '8px', fontSize: '0.8rem', background: 'rgba(255,255,255,0.04)',
                  '& fieldset': { borderColor: 'rgba(255,255,255,0.1)' }, '&:hover fieldset': { borderColor: 'rgba(255,255,255,0.25)' }, '&.Mui-focused fieldset': { borderColor: 'rgba(255,255,255,0.4)' } },
                '& .MuiInputLabel-root': { color: '#777', fontSize: '0.8rem' }, '& .MuiSelect-icon': { color: '#777' } }}>
                <InputLabel>{t('chat.model')}</InputLabel>
                <Select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} label={t('chat.model')}
                  MenuProps={{ PaperProps: { sx: { background: '#111', border: '1px solid rgba(255,255,255,0.1)', color: '#e0e0e0', borderRadius: '10px' } } }}>
                  <MenuItem value="2" sx={{ fontSize: '0.875rem', '&:hover': { background: 'rgba(255,255,255,0.05)' } }}>{t('chat.modelOllama')}</MenuItem>
                  <MenuItem value="3" sx={{ fontSize: '0.875rem', '&:hover': { background: 'rgba(255,255,255,0.05)' } }}>{t('chat.modelClaude')}</MenuItem>
                </Select>
              </FormControl>
              <IconButton onClick={() => setPdfLibraryOpen(true)} size="small"
                sx={{ color: '#777', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', p: 0.75, '&:hover': { color: '#ccc', border: '1px solid rgba(255,255,255,0.3)', background: 'transparent' } }}>
                <FolderIcon sx={{ fontSize: 16 }} />
              </IconButton>
              {attachedDoc && (
                <Chip label={`📄 ${attachedDoc.filename}`} size="small" onDelete={() => setPdfLibraryOpen(true)}
                  sx={{ fontSize: '0.75rem', background: 'rgba(255,255,255,0.06)', color: '#aaa', border: '1px solid rgba(255,255,255,0.12)', '& .MuiChip-deleteIcon': { color: '#666', '&:hover': { color: '#fff' } } }} />
              )}
            </Box>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-end' }}>
              <TextField fullWidth multiline rows={1} value={inputMessage}
                ...
                sx={{ '& .MuiOutlinedInput-root': { color: '#e0e0e0', borderRadius: '14px', background: '#0d0d0d',
                    '& fieldset': { borderColor: 'rgba(255,255,255,0.1)' }, '&:hover fieldset': { borderColor: 'rgba(255,255,255,0.2)' }, '&.Mui-focused fieldset': { borderColor: 'rgba(255,255,255,0.4)' } },
      ...
      <PDFLibrary
        ...
        onAttach={(doc) => {
          ...
          if (currentSessionId && !tempSession) {
            setSessions(prev => ({ ...prev, [currentSessionId]: { ...prev[currentSessionId], attachedDocumentId: doc.id, attachedDocumentName: doc.filename } }))
          }
        }}
        onDetach={() => {
          setAttachedDoc(null)
          if (currentSessionId && !tempSession) {
            setSessions(prev => ({ ...prev, [currentSessionId]: { ...prev[currentSessionId], attachedDocumentId: null, attachedDocumentName: '' } }))
          }
        }}
      />
